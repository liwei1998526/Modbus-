...
char *FUNCTION0F(string recv_str, char* send_buf, int k, vector<int>&val)
{
	...
	string data_recv = recv_str.substr(24, recv_str.size() - 24);
	string data_len = recv_str.substr(22, 2);
	...
	else if (data_len_int * 2 != data_recv.size() || digitlength != data_recv.size())
	{
		string send;
		for (int i = 0; i < 18; i++)
		{
